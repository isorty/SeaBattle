...
namespace Battleships.Model
{
    ...
    public class ServerManager
    {
        ...
        {
            while (tcpClient.Connected && Battle.Info.Status != BattleState.End)
            {
                try
                {
        ...
        private async Task AttackRequestMessageRecived(string message)
        {
            ...
            await SendMessage(MessageType.AttackResult, Battle.Player.PlayerField.GetAttackOnFieldResult(i,j));
        }
        ...
        private async Task AttackResultMessageRecived(string message)
        {
            ...
            if (atckMsg.Result == AttackResult.Miss) await Battle.Enemy.GiveTurn();

            Battle.Player.EnemyField.ReactToAttackResult(atckMsg);
        }
